  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: DashboardScaffold(
        currentRoute: 'delivery_zones',
        title: 'مناطق التوصيل',
        subtitle:
            'عرّف المدن والمناطق التي يصل إليها المتجر مع تسعير واضح وسهل التعديل.',
        child: BlocConsumer<DeliveryZonesCubit, DeliveryZonesState>(
          listener: (BuildContext context, DeliveryZonesState state) {
          ...
          builder: (BuildContext context, DeliveryZonesState state) {
            final DeliveryZonesCubit cubit = context.read<DeliveryZonesCubit>();
            final bool isBusy =
                state is DeliveryZonesLoading || state is DeliveryZonesSaving;

            if (state is DeliveryZonesLoading && cubit.deliveryZones.isEmpty) {
            ...
            return Row(
              ...
              children: <Widget>[
                Expanded(
                  flex: 3,
                  child: DeliveryZonesListPanel(cubit: cubit, isBusy: isBusy),
                ),
                const SizedBox(width: 20),
                Expanded(
                  flex: 2,
                  child: DeliveryZonesFormPanel(cubit: cubit, isBusy: isBusy),
                ),
              ],
            );
          },
